...
function serialize_RetailCrm_Api_Model_Response_Inventories_InventoriesUploadResponse(RetailCrm\Api\Model\Response\Inventories\InventoriesUploadResponse $model, bool $useStdClass = true)
{
...
if (null !== $model->notFoundOffers[$index18]->stores->quantity) {
    $jsonData["notFoundOffers"][$index18]["stores"]["quantity"] = $model->notFoundOffers[$index18]->stores->quantity;
}
if (null !== $model->notFoundOffers[$index18]->stores->purchasePrice) {
    $jsonData["notFoundOffers"][$index18]["stores"]["purchasePrice"] = $model->notFoundOffers[$index18]->stores->purchasePrice;
}
if (null !== $model->notFoundOffers[$index18]->stores->store) {
    $jsonData["notFoundOffers"][$index18]["stores"]["store"] = $model->notFoundOffers[$index18]->stores->store;
}

if (0 === \count($jsonData["notFoundOffers"][$index18]["stores"])) {
    $jsonData["notFoundOffers"][$index18]["stores"] = $emptyObject;
}
